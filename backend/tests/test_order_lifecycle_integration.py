...
def _load_menu_backend():
    ...
    spec = importlib.util.spec_from_file_location("integration_menu_backend", main_path)
...
def menu_backend(monkeypatch):
    ...
    server = uvicorn.Server(uvicorn.Config(menu.app, host="127.0.0.1", port=port, log_level="warning"))
    ...
    monkeypatch.setattr(config, "MENU_BACKEND_URL", base_url)
    yield base_url
    server.should_exit = True
...
def test_order_lifecycle_over_real_menu_backend(menu_backend, tool_graph):
    ...
    state = invoke_tool(tool_graph, state, "add_cart", {
        "item_id": "item-001",
        "title": "Chicken Biryani",
        "new_item": {"key": "item-001|no_variant", "quantity": 2, "base_price": 220.0},
    }, "add-item")
    assert state["cart"].items[0].units[0].quantity == 2

    # 2. place_order sends an actual POST to the running menu backend.
    state = invoke_tool(tool_graph, state, "place_order", {}, "place-order")
    ...
    # 3. Status lookup fetches the same persisted order through an actual GET.
    state_before_status = dict(state)
    state = invoke_tool(tool_graph, state, "get_order_status", {}, "confirmed-status")
    ...
    # 4. Cancellation sends an actual DELETE and changes only lifecycle status.
    state_before_cancel = dict(state)
    state = invoke_tool(tool_graph, state, "cancel_order", {}, "cancel-order")
    ...
    # 5. The persisted order remains retrievable and reports the cancelled status.
    state = invoke_tool(tool_graph, state, "get_order_status", {}, "cancelled-status")
    assert "Order " + order_id + " is cancelled." in state["messages"][-1].content
    assert "Status: cancelled" in state["messages"][-1].content

    # A real backend 404 must not alter the existing lifecycle state.
    before_missing_cancel = dict(state)
    state = invoke_tool(tool_graph, state, "cancel_order", {"order_id": "ORD-MISSING"}, "missing-cancel")
