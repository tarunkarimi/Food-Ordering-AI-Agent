...
def cancel(state, args=None):
    return run_tool(state, "cancel_order", args or {}, "test-cancel-order")
...
def test_status_lookup_renders_cancelled_status():
    state = make_state(make_cart())
    state["orderId"] = "ORD-STATE"
    cancelled_order = order_response(payload={
        "order_id": "ORD-STATE", "restaurant_name": "Test Restaurant", "subdomain": "test",
        "items": [], "subtotal": 0.0, "status": "cancelled",
    })
    with patch("src.agents.tools.order.requests.get", return_value=cancelled_order):
        result = lookup(state)
...
def test_successful_cancellation_updates_status_and_preserves_order_state():
    ...
    with patch(
        ...
        return_value=order_response(payload={"order_id": "ORD-123", "status": "cancelled"}),
    ) as delete:
        result = cancel(state)
    assert delete.call_args.args[0].endswith("/orders/ORD-123")
...
def test_cancel_without_order_id_preserves_state():
    state = make_state(make_cart())
    before = state.copy()
    with patch("src.agents.tools.order.requests.delete") as delete:
        result = cancel(state)
...
def test_cancel_failure_paths_preserve_state():
    failures = [
        ...
        requests.Timeout,
        order_response(payload={"status": "cancelled"}),
    ]
    for failure in failures:
        ...
        state["order_status"] = "confirmed"
        before = state.copy()
        side_effect = failure if isinstance(failure, type) else None
        with patch(
            "src.agents.tools.order.requests.delete",
            ...
            result = cancel(state)
        assert "Unable to cancel order" in tool_content(result) or "was not found" in tool_content(result)
        assert_state_unchanged(before, result)
